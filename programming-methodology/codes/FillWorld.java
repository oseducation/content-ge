...
public class FillWorld extends SuperKarel {
    ...
    private void goUp() {
        if (facingEast()) {
            turnLeft(); //თუ მარცხნიდან მარჯვნივ ვმოძრაობდით
        } else {
            turnRight(); // თუ მარჯვნიდან მარცხნივ მოვედით
        }

        if (frontIsClear()) {
            move();
            if (leftIsBlocked()) {
                turnRight(); //მარცხენა კედლის შემთხვევაში ვუხვევთ მარჯვნივ
            } else {
                turnLeft(); // მარჯვენას შემთხვევაში მარცხნივ
            }
        }
    }
    ...
    private void fillOneLine() {
        putBeeperSafely(); // off by one ბაგისთვის
        while (frontIsClear()) {
            move();
            putBeeperSafely();
        }
